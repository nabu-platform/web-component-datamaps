window.addEventListener("load", function () {
	/**
	Modified to support both d3 v3 (original) and d3 v5. Not tested with d3 v4.
	Most features seem to work but it has not been extensively tested.
	*/
	(function() {
	  var svg;
	  ...
	  var defaultOptions = {
		...
		/*
			Included zoom code that has also been modified to support v3 and v5
		*/
	//basic map config with custom fills, mercator projection
	function Zoom(args) {
		this.$buttons = document.querySelectorAll(".zoom-button");
		this.$info = document.querySelector("#zoom-info");
		this.scale = { max: 50, currentShift: 0 };
		this.$container = args.$container;
		this.datamap = args.datamap;
		this.init();
	}
	Zoom.prototype.init = function() {
		...
		this.d3Zoom = (d3.behavior ? d3.behavior.zoom : d3.zoom)().scaleExtent([ 1, this.scale.max ]);
		this._displayPercentage(1);
		this.listen();
	};
	Zoom.prototype.listen = function() {
		var self = this;
		this.$buttons.forEach(function(x) { x.addEventListener("click", self._handleClick.bind(self)) });
		this.datamap.svg
	...
	Zoom.prototype._handleScroll = function() {
		...
		// v5
		if (!translate && d3.event.transform) {
	...
	Zoom.prototype._update = function(translate, scale) {
		// this is for v3 version, in the v5 version, the transform is enough at least, did not dig into why it does this for v3
		if (this.d3Zoom.translate) {
			this.d3Zoom
				.translate(translate)
				.scale(scale);
		}
	...
	Zoom.prototype._getNextScale = function(direction) {
	  ...
	  */
	  ...
	  function setProjection( element, options ) {
	    ...
	    if ( options.scope === 'usa' ) {
	      projection = (d3.geo ? d3.geo.albersUsa() : d3.geoAlbersUsa())
		.scale(width)
		.translate([width / 2, height / 2]);
	    }
	    else if ( options.scope === 'world' ) {
	    	var type = "geo" + options.projection.substring(0, 1).toUpperCase() + options.projection.substring(1);
	      projection = (d3.geo ? d3.geo[options.projection] : d3[type])()
	    ...
	    path = (d3.geo ? d3.geo.path() : d3.geoPath())
	  ...
	  function handleBubbles (layer, data, options ) {
		...
		.attr('r', function(datum) {
		  ...
		  return d3.geo && options.animate ? 0 : val(datum.radius, options.radius, datum);
		})
		.attr('data-info', function(datum) {
		  return JSON.stringify(datum);
		})
	  ...
	  ***************************************/
	  ...
	  function Datamap( options ) {
	    ...
	    if (!d3.geo || d3.select( this.options.element ).select('svg').length > 0 ) {
	  ...
	  ***************************************/
	  ...
	  Datamap.prototype.updatePopup = function (element, d, options) {
	    ...
	    element.on('mousemove', function() {
	      ...
	      d3.select((self.svg._groups ? self.svg._groups[0][0] : self.svg[0][0]).parentNode).select('.datamaps-hoverover')
